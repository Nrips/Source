...
{
    self.resizing = NO;
    CGRect frame = self.cropRectView.frame;
    //DLog(@"Image cropRectView frame:%@", NSStringFromCGRect(self.cropRectView.frame));

    if (frame.size.height < 250 || frame.size.width < 250) {
        frame.size = CGSizeMake(250, 250);
        self.cropRectView.frame = frame;
    }
    //DLog(@"Image cropRectView frame After checking minimum image size:%@", NSStringFromCGRect(self.cropRectView.frame));
